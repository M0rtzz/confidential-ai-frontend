...
export const importUserEncryptionKey = async (file: File, password: string) => {
  ...
  if (!currentScope) throw new Error('当前登录账号缺少客户密钥作用域，请重新登录');
  if (
    ...
    throw new Error('密钥文件格式不匹配');
  }
  // 备份文件的账号作用域随原实例的 ownerId 生成，迁移到本实例后必然不同。
  // 解密仍以文件内记录的作用域参与 AAD 校验，口令或文件被篡改一样会失败；
  // 校验通过后按当前账号作用域落库，恢复出的 kid 与资产 recipientKid 保持一致。
